...
function show_table() {
	if(isset($_REQUEST['leadid']) && isset($_REQUEST['form'])) {
			...
			echo "<div class='wrap'>Dentro de wrappp:::".apply_filters('kws_gf_directory_detail', apply_filters('kws_gf_directory_detail_'.(int)$_REQUEST['leadid'], GFDirectory::process_lead_detail(false, '', apply_filters('kws_gf_directory_showadminonly_lightbox', apply_filters('kws_gf_directory_showadminonly_lightbox_'.$_REQUEST['form'], $transient, (int)$_REQUEST['leadid']), (int)$_REQUEST['leadid']), (int)$_REQUEST['leadid']), (int)$_REQUEST['leadid']), (int)$_REQUEST['leadid'])."</div>";
	}
}
