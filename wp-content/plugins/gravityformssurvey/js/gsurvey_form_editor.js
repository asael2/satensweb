...
function SetDefaultValues_likert(field) {
    ...
    field.choices = new Array(
        new Choice(gsurveyLikertStrings.columnLabel1, GenerateLikertChoiceValue(field), false),
        new Choice(gsurveyLikertStrings.columnLabel2, GenerateLikertChoiceValue(field), false),
        new Choice(gsurveyLikertStrings.columnLabel3, GenerateLikertChoiceValue(field), false),
        new Choice(gsurveyLikertStrings.columnLabel4, GenerateLikertChoiceValue(field), false),
        new Choice(gsurveyLikertStrings.columnLabel5, GenerateLikertChoiceValue(field), false)
    );
...
jQuery(document).bind("gform_load_field_settings", function (event, field, form) {
    ...
    if (field.inputType == 'likert') {
        ...
        if (field.gsurveyLikertEnableMultipleRows == undefined)
            field.gsurveyLikertEnableMultipleRows = false;
        var gsurveyLikertEnableMultipleRows = field.gsurveyLikertEnableMultipleRows;
        jQuery('#gsurvey-likert-enable-multiple-rows').prop('checked', gsurveyLikertEnableMultipleRows);

        jQuery("#gsurvey-field-type").val('likert');

        if (gsurveyLikertEnableMultipleRows)
...
function gsurveyLikertUpdateColumnsObject() {
    ...
    jQuery('ul#gsurvey-likert-columns li').each(function (index) {
        var text = jQuery(this).children('input.gsurvey-likert-column-text').val();
        var val = jQuery(this).children('input.gsurvey-likert-column-value').val();
        var i = jQuery(this).data("index");
        var g = new Choice(text, val);
        field.choices[i] = g;
    });
    gsurveyLikertUpdateInputs(field);
